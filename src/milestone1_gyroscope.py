# -*- coding: utf-8 -*-
"""
Created on Sun Oct 28 11:05:04 2018
...
# 2. Filter columns
df_columns_filtered = df_2_days[[c for c in df if (c.startswith('Gyroscope')and not c.endswith('FFT'))]]
...
exclude = [] # Add columns to be removed
...
X_pca = estimator.fit_transform(df_correlation.transpose())
...
ax.scatter(df_pca.iloc[:,0], df_pca.iloc[:,1], marker='o', c='red')
plt.show()
...
plt.plot(range(min_sd,max_sd), distortions, marker='o', c='blue')
plt.plot(range(min_sd,max_sd), silhouettes , marker='o', c='green')
plt.xlabel('Number of clusters')
plt.ylabel('Silhouette (G) / Distorsion (B)')
plt.show()
